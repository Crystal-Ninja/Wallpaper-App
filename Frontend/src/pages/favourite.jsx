...
export default function Favourite() {
  ...
  const fetchFavorites = async () => {
    try {
      ...
      const res = await axios.get("http://localhost:5000/images/all-favorites", {
  ...
  const removeFavorite = async (favorite) => {
    try {
      ...
      if (favorite.type === 'external') {
        ...
        await axios.delete(`http://localhost:5000/images/external/${favorite.id}/favorite`, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
        // Remove internal favorite
        await axios.delete(`http://localhost:5000/images/${favorite.id}/favorite`, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
        ...
        >
          {favorites.map((fav) => (
            <div key={`${fav.type}-${fav.id}`} className="relative group mb-4">
              ...
              <div className="absolute top-2 left-2 bg-black/60 text-white px-2 py-1 rounded text-xs opacity-0 group-hover:opacity-100 transition-opacity">
